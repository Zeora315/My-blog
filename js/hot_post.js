(function () {
  ...
  function readConfig() {
    const element = document.getElementById("solitude-hot-posts-data");
  ...
  function getList() {
    return document.querySelector(".hot-post-page .hot-post-list, .card-hot-post .hot-post-list");
  }
  ...
  function render(items, fallbackPosts, limit) {
    const list = getList();
    if (!list) return;
  ...
  async function loadRanking(config) {
    ...
    render(data.items || [], config.posts.slice(0, config.limit), config.limit);
  }
