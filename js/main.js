...
const scrollDamping = (() => {
  ...
  const hasScrollableParent = (element, deltaY) => {
    ...
    for (let node = element; node && node !== document.body; node = node.parentElement) {
      ...
      if (direction > 0 && node.scrollTop + node.clientHeight < node.scrollHeight) return true;
      if (direction < 0 && node.scrollTop > 0) return true;
    }
    return false;
  };

  const canDampTarget = (event, deltaY) => {
    if (event.ctrlKey || event.metaKey || event.defaultPrevented) return false;
    const target = event.target instanceof Element ? event.target : null;
    if (!target) return true;
    if (hasScrollableParent(target, deltaY)) return false;
    return !target.closest(
      "#rightMenu, #console, .aplayer, .fancybox__container, [data-scroll-damping-ignore]"
    );
  };
  ...
  const onWheel = (event) => {
    ...
    if (!state.enabled || !canDampTarget(event, deltaY)) return;
    event.preventDefault();
    state.target = clamp(state.target + deltaY * 1.35);
    if (!state.frame) animate();
  };

  const onScroll = () => {
    if (!state.enabled || state.locked) return;
    state.target = state.current = window.scrollY || document.documentElement.scrollTop;
  };

  const bind = () => {
    if (document.documentElement.dataset.scrollDampingBound === "true") return;
    document.documentElement.dataset.scrollDampingBound = "true";
  ...
  return {
    set(enabled) {
      ...
      document.documentElement.classList.toggle("scroll-damping", state.enabled);
      state.target = state.current = window.scrollY || document.documentElement.scrollTop;
      if (!state.enabled && state.frame) {
        cancelAnimationFrame(state.frame);
        state.frame = 0;
      }
      if (state.enabled) bind();
    },
    get enabled() {
...
const actions = {
  ...
  syncScrollDampingState(enabled) {
    document
      .querySelectorAll("#consoleScrollDamping, .scroll-damping-switch")
      .forEach((item) => item.classList.toggle("on", enabled));
  },
  initScrollDamping() {
    const config = Solitude.config.scroll_damping;
    if (!config?.enable) return;
    const saved = Solitude.saveToLocal.get("scroll-damping");
    const enabled = saved === undefined ? config.default : Boolean(saved);
    scrollDamping.set(enabled);
    this.syncScrollDampingState(enabled);
  },
  switchScrollDamping() {
    if (!Solitude.config.scroll_damping?.enable) return;
    const enabled = !scrollDamping.enabled;
    scrollDamping.set(enabled);
    this.syncScrollDampingState(enabled);
    Solitude.saveToLocal.set("scroll-damping", enabled, 365);
    const lang = Solitude.config.lang.scroll_damping || {};
    Solitude.snackbarShow(enabled ? lang.open : lang.close, false, 2000);
  },
  initConsoleState() {
    const consoleHideAside = document.querySelector("#consoleHideAside");
    ...
    this.syncScrollDampingState(scrollDamping.enabled);
  },
  changeWittyWord() {
...
window.sco = Object.assign(window.sco || {}, {
  toTop: () => Solitude.toTop(),
  showConsole: () => Solitude.showConsole(),
  hideConsole: () => Solitude.hideConsole(),
  switchKeyboard: () => Solitude.switchKeyboard(),
  scrollTo: (id) => Solitude.scrollTo(id),
});
window.toRandomPost = () => Solitude.randomPost();
window.travelling = () => Solitude.travelling?.();
...
Solitude.refresh = async () => {
  ...
  if (page === "archive") {
    const { archivePageController, initArchivePage, initContribWall } = await import("./archive-page.js");
    initContribWall();
    initArchivePage();
  ...
  initAboutCardGlow();
  forPostFn();
  Solitude.syncScrollDampingState(scrollDamping.enabled);
};
